...
use super::{Not, NotEntries, PaddingUnbound};

impl<'a, I, A> IntoIterator for Not<I>
where
    ...
    NotEntries<I::IntoIter, A>: Iterator<Item = bit_set::Entry<A>>,
{
    ...
    fn into_iter(self) -> Self::IntoIter {
        NotEntries {
            padding: PaddingUnbound::new(0, self.val.into_iter()),
            _block: PhantomData,
        }
    }
}
    ...
    A: Iterator<Item = bit_set::Entry<Cow<'a, [T]>>>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.padding.next().map(|page| bit_set::Entry {
            index: page.index,
            block: !page.block,
        })
    }
