...
macro_rules! impls {
    ($($ty:ty),*) => ($(
        ...
        impl Assign<std::ops::Range<u64>> for $ty {
            type Output = u64;

            fn set1(&mut self, r: std::ops::Range<u64>) -> Self::Output {
                let i = r.start;
                let j = r.end;
                if i >= j {
                    0
            ...
            fn set0(&mut self, r: std::ops::Range<u64>) -> Self::Output {
                let i = r.start;
                let j = r.end;
                if i >= j {
                    0
...
impl<U: UnsignedInt> Assign<std::ops::Range<u64>> for Block<U>
where
    [U]: Assign<std::ops::Range<u64>, Output = u64>,
{
    type Output = u64;
